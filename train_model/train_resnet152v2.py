...
from model_densenet121 import model_fn
...
def train_input_fn(training_dir, hyperparameters):
    ...
    ds_val = _input(tf.estimator.ModeKeys.EVAL, batch_size=BATCH_SIZE, data_df=val_df)
...
def eval_input_fn(evaluation_dir, hyperparameters):
    ...
    ds_eval = _input(tf.estimator.ModeKeys.EVAL, batch_size=BATCH_SIZE, data_df=df_eval)
...
def test_input_fn(training_dir, hyperparameters):
    ...
    ds_test = _input(tf.estimator.ModeKeys.EVAL, batch_size=BATCH_SIZE, data_df=df_test)
    
    return ds_test, df_test


def _input(mode, batch_size, data_df):
...
def generate_confusion_matrix_plot(num_label, Y_test, pred_labels, ds_test, df_test, model):
    ...
    cm = pd.DataFrame(confusion_matrix , index = ['1','0'] , columns = ['0','1'])
...
def generate_learning_curve_loss_plot(history, hyperparameters):
    ...
    image_name = 'learning_curve_loss.png'
    image_path = "/opt/ml/output/"
    plt.savefig(image_path + image_name)
    upload_image_to_s3(image_path, image_name)
...
def generate_learning_curve_accuracy_plot(history, hyperparameters):
    ...
    ax.legend(['train', 'val'], loc='best')
    image_name = 'learning_curve_accuracy.png'
    image_path = "/opt/ml/output/"
    plt.savefig(image_path + image_name)
    upload_image_to_s3(image_path, image_name)
...
if __name__ == "__main__":
    ...
    print(metrics.classification_report(Y_test, pred_labels, target_names = ['Pneumonia (Class 0)','Normal (Class 1)']))
